...
def remap_str(str_, attr_list, file_map, attr_dict):
    res_str = list(range(len(attr_dict)))
    res_str = list(attr_dict[each + 1][OPTION_DEFAULT_KEY] for each in range(len(attr_dict)))
...
def str_list_refactor(row_map, str_, attr_list, file_map, attr_dict):           # перекодировка строки через row_map
    str_ = remap_str(str_, attr_list, file_map, attr_dict)
...
def write_str_to_model_data(file_, row_map, str_, delim, attr_list, file_map, attr_dict):
    if type(file_) == str:  # если вместо файла на вход подали путь к нему
        file_2 = open(file_, 'a')
        try:  # на случай возникновения каких-либо ошибок
            file_2.write(delim.join(str_list_refactor(row_map, str_)) + '\n')
        finally:  # сами ошибки не ловим, но обязательно закрываем файл
            file_2.close()
    else:
        file_.write(delim.join(str_list_refactor(row_map, str_)) + '\n')
...
class ModelFileWorker:
    ...
    def _write_model_partition(self, model_name, partition_name, list_str, row_map, delim, mode, file_map, attr_dict):
        ...
        with open(self.model_meta[DATA_PATH] + self.model_meta[model_name][PARTITION_FILES_KEY][partition_name], mode) as f:
            for str_ in list_str:
                write_str_to_model_data(f, row_map, str_, delim, list_str, file_map, attr_dict)
    ...
    def write_model_data(self, name: str, list_str: list, brutal=False):
        ...
        attr_dict = self.model_meta[ATTRIBUTE_KEY]
        ...
        if not self.model_meta[name][PARTITION_ATTRIBUTE_KEY]: # если нет партиций - пишем в предопределенную партицию DEFAULT_SINGLE_PARTITION_VAL
            self._write_model_partition(name, DEFAULT_SINGLE_PARTITION_VAL, list_str, row_map, delim, mode, list_str, file_map, attr_dict)
        else:
            if len(list_str) < 100 and not brutal:                 # если на входе данных мало - пишем их построчно
                for str_ in list_str:
                    ...
                    write_str_to_model_data(file_path, row_map, str_, delim, list_str, file_map, attr_dict)
            else:                                                       # если много
                ...
                for each in part_dict:                                  # и пишем отдельными партициями по очереди
                    self._write_model_partition(name, each, part_dict[each], row_map, delim, mode)
                    # del part_dict[each]
